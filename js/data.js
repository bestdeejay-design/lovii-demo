...
const LOVII_PAY_SEED = {
  ...
  // ---- Карты программы (канон владельца, 2026-09-12) ----
  // У клиента внутри платформы ОДИН счёт и ОДИН номер — меняется только
  // скин карты: LOVII PAY (база, всем при регистрации) → LOVII PASS
  // (премиум, по подписке; даёт статус представителя LOVII и вывод
  // баллов на карту без комиссий от 3 000 ₽) → LOVII VIP (оборот от
  // 300 000 ₽ В МЕСЯЦ). Четвёртый скин — LOVII BUSINESS: выдаётся по
  // умолчанию всем МСП (торговым точкам), виден в профиле, показывает
  // операции по счёту МСП (см. LOVII_PAY_SEED.biz).
  cardSkins: [
  ...
  // ---- Правила вывода (канон владельца, 2026-09-12) ----
  // LOVII PASS и выше: вывод баллов НА КАРТУ без комиссий от 3 000 ₽.
  // Базовый PAY: вывод через СБП — правила вывода не заданы владельцем
  // (беклог lovii_docs/canon/BACKLOG.md §6, не выдумывать числа).
  ...
  // НОМЕР ЕДИНЫЙ ДЛЯ ВСЕХ ЛИЧНЫХ СКИНОВ (PAY/PASS/VIP) — счёт один;
  // смена статуса меняет только оформление. Задача генерации номеров
  // готовится отдельно — меняем только эти поля.
  card: {
  ...
  // pay  — базовый уровень, есть у всех (выдаётся при регистрации);
  // pass — по действующей подписке Лови (bySub, бинарное условие);
  //        даёт статус представителя LOVII + вывод баллов на карту
  //        без комиссий от 3 000 ₽ (канон владельца, 2026-09-12);
  // vip  — по обороту по карте: владелец (2026-09-12) — «оборот LOVII
  //        от 300 000 рублей В МЕСЯЦ» — период зафиксирован: месяц.
  tiers: [
    { id: 'pay',  name: 'LOVII PAY',  need: 0,     bySub: false, cond: 'базовый уровень · выдаётся при регистрации',
    ...
    { id: 'pass', name: 'LOVII PASS', need: 0,     bySub: true,  cond: 'по подписке Лови',
      perks: ['Статус представителя LOVII', 'Вывод на карту 0% от 3 000 ₽', 'Мерч Лови', 'Закрытые мероприятия', 'Спец-скидки у МСП'] },
    { id: 'vip',  name: 'LOVII VIP',  need: 300000, bySub: false, cond: 'оборот от 300 000 ₽ в месяц',
  ...
  // ---- LOVII BUSINESS: бизнес-карта МСП (канон владельца, 2026-09-12) ----
  // Выдаётся по умолчанию всем МСП (торговым точкам), видна в профиле;
  // счёт у точки СВОЙ (не личный) — здесь отображаются все операции
  // по счёту МСП. Демо-точка — «Кофейня «Daily»» (точка №1019).
  // Номер бизнес-карты: тот же стандарт (префикс 9643), пул эмитента
  // 9142 — бизнес (демо), Лун: OK. Тариф LOVII BUSINESS — беклог владельца.
  biz: {
    name: 'Кофейня «Daily»',
    point: 'Точка №1019 · Тверской',
    card: { bin: '9643 9142', tail: '8802 3019', holder: 'DAILY · 1019' },
    monthTurnover: 412300, // оборот точки за месяц, ₽ (демо)
    // Операции по счёту МСП: kind = in (поступление) | client (баллы клиентов) | out (выплаты)
    opsSeed: [
      { d: 0, kind: 'in',    title: 'Выручка · оплата QR',            sum: 18450,  note: '34 покупки за день' },
      { d: 0, kind: 'client',title: 'Приём баллов · погашение',       sum: 1250,   note: 'Компенсация баллов от Лови' },
      { d: 1, kind: 'in',    title: 'Выручка · оплата QR',            sum: 21030,  note: '41 покупка за день' },
      { d: 1, kind: 'client',title: 'Начислено баллов клиентам',      sum: -840,   note: 'Кэшбек по программе 1:1' },
      { d: 2, kind: 'out',   title: 'Выплата на расчётный счёт',      sum: -120000, note: 'Регистрация · завтра в 10:00' },
      { d: 3, kind: 'in',    title: 'Выручка · оплата QR',            sum: 16890,  note: '29 покупок за день' },
      { d: 4, kind: 'client',title: 'Приём баллов · погашение',       sum: 940,    note: 'Компенсация баллов от Лови' },
      { d: 5, kind: 'out',   title: 'Комиссия платформы',             sum: -1690,  note: 'По тарифу LOVII BUSINESS' },
    ],
  },
};
